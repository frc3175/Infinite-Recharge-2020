...
public class Limelight {
    ...
    private Drive limelightDrive = new Drive();
    ...
    public void limelightTest(boolean pressed) {
        if(pressed) {
            if(x < 0)
            limelightDrive.limelightTank(0, 0.1);
        } else if (x > 0) {
            limelightDrive.limelightTank(0.1, 1);
        }
    }
    ...
    //             double currentDistance = Mathematics.countDistance(y, LimelightConstants.heightDifference); // Distance
    //                                                                                                         // from
    //                                                                                                         // target
    //             double distanceDifference = Mathematics.calcPulses(LimelightConstants.TrenchDistance)
    //                     - Mathematics.calcPulses(currentDistance); // Difference in distance (error)
    //             double distanceAdjust = distanceDifference / LimelightConstants.navigationTime; // Calculates a distance
    //                                                                                             // adjustment based on
    //                                                                                             // error
    //             double steeringAdjust = LimelightConstants.angularScaleUp * x; // Creates a side-to-side adjustment
    //                                                                            // based on error
    //             Drive.flyWithWires(Drive.m_rightDriveMaster, Drive.m_leftDriveMaster, steeringAdjust,
    //                     distanceAdjust * LimelightConstants.distanceScaleUp); // Drive using adjustment values

    //         } else if (pressedLine) {
    //             driveMode = 1;
    //             table.getEntry("camMode").setNumber(0); // Tracking mode
    //             table.getEntry("ledMode").setNumber(3); // LEDs on
    //             table.getEntry("pipeline").setNumber(0); // Pipeline 2

    //             if (limelight.hasAnyTarget()) {
    //                 double currentDistance = Mathematics.countDistance(y, LimelightConstants.heightDifference); // Distance
    //                                                                                                             // from
    //                                                                                                             // target
    //                 double distanceDifference = Mathematics.calcPulses(LimelightConstants.LineDistance)
    //                         - Mathematics.calcPulses(currentDistance); // Difference in distance (error)
    //                 double distanceAdjust = distanceDifference / LimelightConstants.navigationTime; // Calculates a
    //                                                                                                 // distance
    //                                                                                                 // adjustment based
    //                                                                                                 // on error
    //                 double steeringAdjust = LimelightConstants.angularScaleUp * x; // Creates a side-to-side adjustment
